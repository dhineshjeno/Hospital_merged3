...
async function getPatientDemographics(hospitalId, dateFrom, dateTo) {
  const [totalsResult, genderResult, ageResult, bloodGroupResult, statusResult] = await Promise.all([
    ...
    query(
      `SELECT gender, COUNT(*)::int AS count,
       ...
       FROM patients WHERE hospital_id = $1 GROUP BY gender ORDER BY count DESC`,
      [hospitalId],
    ),
    query(
      `SELECT
         ...
         COUNT(*)::int AS count
       FROM patients
       WHERE hospital_id = $1 AND status != 'deceased'
       GROUP BY age_group
       ORDER BY age_group`,
      [hospitalId],
    ),
    query(
      `SELECT blood_group, COUNT(*)::int AS count
       FROM patients WHERE hospital_id = $1 AND blood_group IS NOT NULL
    ...
    query(
      `SELECT
       ...
       FROM patients WHERE hospital_id = $1 AND status = 'active'`,
      [hospitalId, dateFrom],
    ),
  ]);
...
async function getAppointmentAnalytics(hospitalId, dateFrom, dateTo) {
  const [totalsResult, byStatusResult, byDoctorResult, byTypeResult, byDayResult] = await Promise.all([
    query(
      `SELECT
       ...
       WHERE p.hospital_id = $1 AND (a.scheduled_start_at AT TIME ZONE 'Asia/Kolkata')::date BETWEEN $2 AND $3`,
      [hospitalId, dateFrom, dateTo],
    ),
    query(
    ...
    query(
      `SELECT d.doctor_id, d.first_name || ' ' || d.last_name AS doctor_name,
       ...
       FROM doctors d
       JOIN doctor_schedules ds ON ds.doctor_id = d.doctor_id
       LEFT JOIN appointments a ON a.doctor_id = d.doctor_id
         AND (a.scheduled_start_at AT TIME ZONE 'Asia/Kolkata')::date BETWEEN $2 AND $3
       WHERE ds.hospital_id = $1
    ...
    query(
      `SELECT appointment_type, COUNT(*)::int AS count
       ...
       JOIN patients p ON p.patient_id = a.patient_id
       WHERE p.hospital_id = $1 AND (a.scheduled_start_at AT TIME ZONE 'Asia/Kolkata')::date BETWEEN $2 AND $3
       GROUP BY a.appointment_type ORDER BY count DESC`,
      [hospitalId, dateFrom, dateTo],
    ),
    query(
...
async function getDoctorUtilisation(hospitalId, dateFrom, dateTo) {
  const result = await query(
    `SELECT
     ...
     LEFT JOIN consultations c ON c.doctor_id = d.doctor_id
       AND c.consultation_started_at::date BETWEEN $2 AND $3
     LEFT JOIN prescriptions p ON p.doctor_id = d.doctor_id
       AND p.prescribed_at::date BETWEEN $2 AND $3
     LEFT JOIN lab_orders lo ON lo.doctor_id = d.doctor_id
       AND lo.ordered_at::date BETWEEN $2 AND $3
...
async function getMedicineUsage(hospitalId, dateFrom, dateTo, limit) {
  const [topPrescribedResult, dispensingResult, categoryResult] = await Promise.all([
    query(
      `SELECT
       ...
       WHERE pat.hospital_id = $1 AND p.prescribed_at::date BETWEEN $2 AND $3
       GROUP BY pi.medicine_name
       ORDER BY times_prescribed DESC
       LIMIT $3`,
      [hospitalId, dateFrom, dateTo, limit],
    ),
    query(
      `SELECT
       ...
       WHERE m.hospital_id = $1 AND t.performed_at::date BETWEEN $2 AND $3
...
async function getRevenueReport(hospitalId, dateFrom, dateTo) {
  const [summaryResult, byServiceResult, byDoctorResult, byStatusResult, dailyResult] = await Promise.all([
    query(
      `SELECT
       ...
       WHERE p.hospital_id = $1 AND i.invoice_date BETWEEN $2 AND $3`,
      [hospitalId, dateFrom, dateTo],
    ),
    query(
      `SELECT
    ...
    query(
      `SELECT status, COUNT(*)::int AS count,
       ...
       JOIN patients p ON p.patient_id = i.patient_id
       WHERE p.hospital_id = $1 AND i.invoice_date BETWEEN $2 AND $3
       GROUP BY i.status ORDER BY amount DESC`,
      [hospitalId, dateFrom, dateTo],
    ),
    query(
...
async function getLabReport(hospitalId, dateFrom, dateTo) {
  const [summaryResult, byTestResult, abnormalResult, byDoctorResult] = await Promise.all([
    query(
      `SELECT
       ...
       WHERE p.hospital_id = $1 AND lo.ordered_at::date BETWEEN $2 AND $3`,
      [hospitalId, dateFrom, dateTo],
    ),
    query(
      `SELECT
...
async function getQueueAnalytics(hospitalId, dateFrom, dateTo) {
  const [summaryResult, byDoctorResult, byHourResult] = await Promise.all([
    query(
      `SELECT
       ...
       JOIN patients p ON p.patient_id = a.patient_id
       WHERE p.hospital_id = $1 AND q.queue_date BETWEEN $2 AND $3`,
      [hospitalId, dateFrom, dateTo],
    ),
    query(
      `SELECT
...
async function getBedOccupancyTrends(hospitalId, dateFrom, dateTo) {
  const [currentResult, admissionTrendResult, avgStayResult, wardBreakdownResult] = await Promise.all([
    query(
      `SELECT
       ...
       FROM beds b
       JOIN wards w ON w.ward_id = b.ward_id WHERE w.hospital_id = $1 AND w.is_active = true`,
      [hospitalId],
    ),
    query(
      `SELECT
       ...
       FROM admissions a
       JOIN patients p ON p.patient_id = a.patient_id
       WHERE p.hospital_id = $1 AND a.admitted_at::date BETWEEN $2 AND $3
       GROUP BY a.admitted_at::date ORDER BY date`,
      [hospitalId, dateFrom, dateTo],
    ),
    query(
      `SELECT
    ...
    query(
      `SELECT
         ...
         COUNT(a.admission_id) FILTER (
           WHERE a.admitted_at::date BETWEEN $2 AND $3
